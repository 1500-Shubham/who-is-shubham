...
/*
 * MIRA appears as a hologram: a female bust sampled into ~4k particles.
 * The silhouette is drawn on an offscreen 2D canvas, sampled into a point
 * cloud, then rendered with a custom shader that
 *   – assembles her from scattered stardust when the section scrolls in,
 *   – shimmers per-particle,
 *   – repels particles around the cursor (she reacts when you hover).
 */
...
function ParticleFigure({ active, hovered }: FigureProps) {
  ...
  useFrame((state, dt) => {
    ...
    u.uPointer.value.set(wx, wy - grp.current.position.y)
...
export default function MiraHologram() {
  ...
  return (
    ...
    >
      ...
      >
        <ParticleFigure active={inView} hovered={hovered} />
        <HoloBase />
        <ScanRing />
